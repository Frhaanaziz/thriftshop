...
export default async function Home() {
  ...
  const [{ data: products }, { data: stores }] = await Promise.all([
    ...
    supabase.from('stores').select('id, name, description').limit(4),
  ]);
    ...
    <div className="container mb-14">
      <div className="my-28 flex flex-col items-center">
        ...
        <p className="text-muted-foreground text-center text-lg lg:text-2xl mt-2 mb-5">
          <Balancer>
            Buy and sell product from independent brands and stores around the
            world with ease
          </Balancer>
        </p>
      ...
      <div className="mb-16">
        ...
        <p className="text-muted-foreground text-lg text-center mx-auto mb-5">
          Discover amazing thrift finds from unique vintage shops and secondhand
      ...
      <div className="my-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-7 xl:grid-cols-4 ">
          {stores?.map((store, i) => (
            <FeaturedStore key={store.id} store={store} index={i} />
          ))}
        </div>
      </div>
