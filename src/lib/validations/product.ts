...
export const newProductSchema = z
    .object({
        ...
        image: z
            .any()
            .refine((files: File) => files?.length <= 3, 'Cannot upload more than 3 files')
            .refine(
                (files) =>
                    files.length === 0 ||
                    files.reduce((acc: number, file: File) => acc + file.size, 0) <= MAX_FILE_SIZE,
    ...
    .superRefine(({ category, sub_category: subCategoryForm }, ctx) => {
        ...
        categories.forEach((catego, i) => {
            if (category === catego && sub_category.at(i)?.includes(subCategoryForm)) {
                isValid = true;
            }
        });
